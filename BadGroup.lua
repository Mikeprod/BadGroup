...
function BadGroup:GetPetOwner(srcGUID)
	...
	if (numMembers > 0) then
		for i = 1, numMembers do
			...
			if (petGUID and petGUID == srcGUID) then
				local ownerName,ownerRealm = UnitName(groupType .. i)
				ownerRealm=ownerRealm or homeRealm
	...
	if (petGUID and petGUID == srcGUID) then
		local ownerName,ownerRealm = UnitName("player")
		ownerRealm=ownerRealm or homeRealm
		return ownerName.."-"..ownerRealm
	end
end
...
function BadGroup:ScanAuras(unitID, groupType)
	...
	if (UnitAura(unitID, auraName) == auraName) then
		...
		playerRealm=playerRealm or homeRealm
...
function BadGroup:AddTank(tankName)
	...
	local homeRealm = GetRealmName()
	homeRealm = homeRealm:gsub("%s+", "")
	playerName=playerName.."-"..(playerRealm or homeRealm)
	
	if (tankName == playerName or tankName == UnitName("pet") or UnitPlayerOrPetInParty(tankName) or UnitPlayerOrPetInRaid(tankName)) then
...
function BadGroup.Command(str, editbox)
	...
	elseif (str == "auras") then
		...
		BadGroup:Print("options : social, private, debug, status, add, del, wipe, tanks, auras")
	end
end